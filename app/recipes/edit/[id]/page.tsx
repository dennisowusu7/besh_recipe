...
class ImageNode extends DecoratorNode<JSX.Element> {
    ...
    exportJSON(): SerializedImageNode {
        return { ...super.exportJSON(), type: "image", version: 1, src: this.__src };
    }
    ...
    decorate(): JSX.Element {
        ...
        return <img src={this.__src} alt="Recipe content" className="my-3 h-auto max-w-full rounded-md" />;
    }
}
...
const theme = {
    ...
    heading: { h2: "mb-2 text-2xl font-semibold", h3: "mb-2 text-xl font-semibold" },
    list: { ul: "list-disc pl-6", ol: "list-decimal pl-6", listitem: "mb-1", nested: { listitem: "mt-1" } },
    link: "text-blue-300 underline",
};
const editorConfig = {
    namespace: "RecipeEditorEdit",
    theme,
...
const ToolbarPlugin = () => {
    ...
    useEffect(() => editor.registerUpdateListener(({ editorState }) => {
        editorState.read(() => {
            ...
            const nodeType = selection.anchor.getNode().getTopLevelElementOrThrow().getType();
    ...
    const handleBlockTypeChange = (value: string) => {
        editor.update(() => {
            const selection = $getSelection();
            if (!$isRangeSelection(selection)) return;
            if (value === "paragraph") return $setBlocksType(selection, () => $createParagraphNode());
            if (value === "quote") return $setBlocksType(selection, () => new QuoteNode());
            if (value === "h2" || value === "h3") return $setBlocksType(selection, () => $createHeadingNode(value));
        });
    };
    ...
    return (
        <div className="mb-3 flex flex-wrap items-center gap-2 border-b border-slate-600/50 pb-3">
            <select value={blockType} onChange={(e) => handleBlockTypeChange(e.target.value)} className="h-8 rounded border border-slate-600/50 bg-slate-700/30 px-2 text-xs text-slate-300 cursor-pointer hover:bg-slate-700/60">
                <option value="paragraph">Normal</option><option value="h2">Heading 2</option><option value="h3">Heading 3</option><option value="quote">Blockquote</option>
            </select>
            <div className={groupClass}>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "bold")}><FiBold size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "italic")}><FiItalic size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "underline")}><FiUnderline size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "strikethrough")}><FiType size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_TEXT_COMMAND, "code")}><FiCode size={14} /></button>
            </div>
            <div className={groupClass}>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, "left")}><FiAlignLeft size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, "center")}><FiAlignCenter size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(FORMAT_ELEMENT_COMMAND, "right")}><FiAlignRight size={14} /></button>
            </div>
            <div className={groupClass}>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(INSERT_UNORDERED_LIST_COMMAND, undefined)}><FiList size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(INSERT_ORDERED_LIST_COMMAND, undefined)}><FiMenu size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(INSERT_CHECK_LIST_COMMAND, undefined)}>CL</button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(REMOVE_LIST_COMMAND, undefined)}>X</button>
            </div>
            <div className={groupClass}>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(INDENT_CONTENT_COMMAND, undefined)}>{">"}</button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(OUTDENT_CONTENT_COMMAND, undefined)}>{"<"}</button>
                <button type="button" className={buttonClass} onClick={insertOrEditLink}><FiLink size={14} /></button>
                <button type="button" className={buttonClass} onClick={insertImageByUrl}><FiImage size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => fileInputRef.current?.click()}>UP</button>
            </div>
            <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={insertImageByFile} />
            <div className={groupClass}>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(UNDO_COMMAND, undefined)}><FiRotateCcw size={14} /></button>
                <button type="button" className={buttonClass} onClick={() => editor.dispatchCommand(REDO_COMMAND, undefined)}><FiRotateCw size={14} /></button>
            </div>
        </div>
    );
};
...
const ImagePlugin = () => {
    ...
    useEffect(() => editor.registerCommand(
        ...
        (payload) => {
            editor.update(() => $insertNodes([$createImageNode(payload), $createParagraphNode()]));
            return true;
        },
        COMMAND_PRIORITY_EDITOR
    ), [editor]);
...
const EditorInitializePlugin: React.FC<EditorInitializePluginProps> = ({ htmlContent }) => {
    const [editor] = useLexicalComposerContext();
    const hasInitialized = useRef(false);

    useEffect(() => {
        if (hasInitialized.current || !htmlContent) return;
...
export default function EditRecipePage() {
    ...
    const { register, handleSubmit, watch, reset, formState: { isSubmitting } } = useForm<RecipeFormValues>({
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                const recipePayload = await recipeRes.json().catch(() => null);
                const catPayload = await catRes.json().catch(() => null);
                if (!recipeRes.ok) {
                    toast.error(recipePayload?.err || "Failed to load recipe.");
                    return;
                }
                ...
                setPreparationHtml(recipe?.preparationSteps || "");
    ...
    const hasImage = Boolean(watchedValues.image?.[0] || previewImage);
    const hasSteps = preparationHtml.replace(/<[^>]*>/g, "").trim().length > 0;
    const completeness = useMemo(() => scoreRecipeCompleteness(watchedValues, hasSteps, hasImage), [watchedValues, hasSteps, hasImage]);

    const handleEditorChange = (editorState: EditorState, editor: LexicalEditor) => {
        editorState.read(() => setPreparationHtml($generateHtmlFromNodes(editor, null)));
    };

    const onSubmit = async (values: RecipeFormValues) => {
        if (!hasSteps) return toast.error("Preparation steps are required.");
        const recipeData = {
        ...
        const toastId = "update-recipe";
        toast.loading("Updating recipe...", { id: toastId });
        try {
            const res = await fetch(`/api/recipes/${recipeId}`, { method: "PATCH", body: formData });
            const payload = await res.json().catch(() => null);
            if (!res.ok) return toast.error(payload?.err || payload?.message || "Failed to update recipe.", { id: toastId });

            // Delete all existing recipe-ingredients for this recipe
            await fetch(`/api/recipes/${recipeId}/ingredients`, { method: "DELETE" }).catch(() => null);
            ...
            toast.error("Failed to update recipe.", { id: toastId });
        }
    };

    const placeholder = useMemo(() => <p className="pointer-events-none absolute top-3 left-3 text-sm text-slate-500">Write your preparation steps here...</p>, []);
    if (loadingRecipe) return <main className="relative min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-4 py-10"><div className="flex items-center justify-center h-screen"><div className="text-slate-300 text-lg">Loading recipe editor...</div></div></main>;
        ...
        <section className="relative min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-4 py-12 sm:px-6 lg:px-8 overflow-hidden">
            ...
            <div className="relative mx-auto max-w-6xl">
                <div className="mb-10">
                    ...
                    <p className="text-lg text-slate-400">Update your recipe using the same workflow.</p>
                </div>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 rounded-2xl border border-slate-700/50 bg-slate-800/30 backdrop-blur-sm p-8 lg:col-span-2">
                        ...
                        <div>
                            <label className="mb-2 block text-sm font-semibold text-slate-300">Replace Recipe Image (optional)</label>
                            <input 
                                type="file" 
                                accept="image/*" 
                            ...
                            <LexicalComposer initialConfig={editorConfig}>
                                <div className="relative rounded-lg border border-slate-600/50 bg-slate-700/20 overflow-hidden">
                                    ...
                                    <div className="relative">
                                        <RichTextPlugin contentEditable={<ContentEditable className="min-h-64 rounded-md border border-slate-600/50 bg-slate-900/50 p-4 text-slate-100 outline-none focus:border-pink-500/50" />} placeholder={placeholder} ErrorBoundary={LexicalErrorBoundary} />
                                    </div>
                                    <HistoryPlugin /><ListPlugin /><LinkPlugin /><ImagePlugin /><EditorInitializePlugin htmlContent={preparationHtml} /><OnChangePlugin onChange={handleEditorChange} />
                                </div>
                            </LexicalComposer>
                            <p className="mt-2 text-xs text-slate-500">If you don't change the editor content, current saved steps will be used.</p>
                        </div>
