import { ConnectToDB, getErrorMessage, getSuccessMessage } from "@/lib/helpers";
import prisma from "@/prisma";

export const GET = async (req: Request) => {
    try {
        await ConnectToDB();
        const { searchParams } = new URL(req.url);
        const userIdParam = searchParams.get("userId");
        const recipeIdParam = searchParams.get("recipeId");

        const userId = userIdParam ? Number(userIdParam) : undefined;
        const recipeId = recipeIdParam ? Number(recipeIdParam) : undefined;

        if (userIdParam && !Number.isInteger(userId)) {
            return getErrorMessage("Validation Error", "Invalid userId", 400, "Bad Request");
        }
        if (recipeIdParam && !Number.isInteger(recipeId)) {
            return getErrorMessage("Validation Error", "Invalid recipeId", 400, "Bad Request");
        }

        const savedRecipes = await prisma.savedRecipe.findMany({
            where: {
                ...(userId !== undefined ? { userId } : {}),
                ...(recipeId !== undefined ? { recipeId } : {}),
            },
            include: {
                recipe: {
                    select: {
                        id: true,
                        title: true,
                        difficulty: true,
                        imageUrl: true,
                        cuisine: true,
                        createdAt: true,
                        _count: {
                            select: {
                                comments: true,
                                ratings: true,
                                savedRecipes: true,
                            },
                        },
                    },
                },
            },
            orderBy: { savedAt: "desc" },
        });

        // Transform the response to return recipes directly with imageUrl mapped to image
        const recipes = savedRecipes.map((sr) => ({
            ...sr.recipe,
            image: sr.recipe.imageUrl,
        }));

        return getSuccessMessage("Saved recipes retrieved successfully", recipes, 200, "OK");
    } catch (err: unknown) {
        return getErrorMessage("Error occurred", err, 500, "Internal Server Error");
    }
};

export const POST = async (req: Request) => {
    try {
        await ConnectToDB();
        const body = await req.json();
        const { userId, recipeId } = body as { userId?: number; recipeId?: number };

        if (!userId || !recipeId) {
            return getErrorMessage("Validation Error", "userId and recipeId are required", 400, "Bad Request");
        }
        if (!Number.isInteger(userId) || !Number.isInteger(recipeId)) {
            return getErrorMessage("Validation Error", "Invalid userId or recipeId", 400, "Bad Request");
        }

        const existingSave = await prisma.savedRecipe.findUnique({
            where: {
                userId_recipeId: {
                    userId,
                    recipeId,
                },
            },
        });

        if (existingSave) {
            return getErrorMessage("Validation Error", "Recipe already saved by this user", 409, "Conflict");
        }

        const createdSave = await prisma.savedRecipe.create({
            data: {
                userId,
                recipeId,
            },
            include: {
                user: {
                    select: {
                        id: true,
                        name: true,
                        profileImage: true,
                    },
                },
                recipe: {
                    select: {
                        id: true,
                        title: true,
                        difficulty: true,
                        imageUrl: true,
                        cuisine: true,
                    },
                },
            },
        });

        return getSuccessMessage("Recipe saved successfully", createdSave, 201, "Created");
    } catch (err: unknown) {
        return getErrorMessage("Error occurred", err, 500, "Internal Server Error");
    }
};

export const DELETE = async (req: Request) => {
    try {
        await ConnectToDB();
        const { searchParams } = new URL(req.url);
        const userIdParam = searchParams.get("userId");
        const recipeIdParam = searchParams.get("recipeId");

        const userId = userIdParam ? Number(userIdParam) : undefined;
        const recipeId = recipeIdParam ? Number(recipeIdParam) : undefined;

        if (!userId || !recipeId) {
            return getErrorMessage("Validation Error", "userId and recipeId are required", 400, "Bad Request");
        }
        if (!Number.isInteger(userId) || !Number.isInteger(recipeId)) {
            return getErrorMessage("Validation Error", "Invalid userId or recipeId", 400, "Bad Request");
        }

        const existingSave = await prisma.savedRecipe.findUnique({
            where: {
                userId_recipeId: {
                    userId,
                    recipeId,
                },
            },
        });

        if (!existingSave) {
            return getErrorMessage("Not Found", "Saved recipe not found", 404, "Not Found");
        }

        await prisma.savedRecipe.delete({
            where: {
                userId_recipeId: {
                    userId,
                    recipeId,
                },
            },
        });

        return getSuccessMessage("Recipe removed from saved", null, 200, "OK");
    } catch (err: unknown) {
        return getErrorMessage("Error occurred", err, 500, "Internal Server Error");
    }
};
